...
if __name__ == '__main__':
    ...
    parser.add_argument('--tr_bsize', default=512, type=int, help='mini-batch train size (default: 100)')
    ...
    cam_dir = './cam_result/cam_img_%s' % args.dataset
    label_dir = './cam_result/cam_labels_%s' % args.dataset
    ...
    for batch_idx, (x, y) in enumerate(train_loader):
        ...
        for i in range(len(idx)):
            class_loss = comp_class_vec(output[i], num_classes)
            class_loss.backward()
            # 生成cam
            grads_val = grad_block[batch_idx][i].cpu().data.numpy().squeeze()
            fmap = fmap_block[batch_idx][i].cpu().data.numpy().squeeze()
            cam = gen_cam(fmap, grads_val)
            print("标签是：%d" % (y[i] + 1), "预测值是：%d" % (idx[i] + 1))
            show_cam_on_image(image, cam, cam_dir, y[i].numpy() + 1, idx[i] + 1, (batch_idx + 1) * (i + 1))
            # 找到当前样本在原图上的位置，减一是为了使索引从0开始，加5是因为标签外围填充了五层0
            loc_center = (center_location[(batch_idx + 1) * (i + 1) - 1] + 5).astype(np.uint)
            ...
            # 将每张热力图对应的表填图画出来并保存
            map_result(args.dataset, cam_label, label_dir, y[i].numpy() + 1, idx[i] + 1, (batch_idx + 1) * (i + 1))
